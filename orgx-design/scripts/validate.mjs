...
const references = [...text.matchAll(/\]\((references\/[^)]+\.md)\)/g)].map(
  (match) => match[1]
);
...
for (const required of [
  ...
  'references/marketing-product-theater.md',
  'references/widget-sdk.md',
  'references/distribution.md',
]) {
  if (!references.includes(required))
    errors.push(`SKILL.md does not route to ${required}`);
}
